...
for dll_path in os.listdir(libs_path):
    if dll_path.endswith(".dll"):
        # Skip RGeoLib - it will be loaded by Grasshopper or from C:\geolib
        if dll_path.lower().startswith("rgeolib"):
            continue
        dll_path = libs_path / dll_path[:-4]
        clr.AddReference(str(dll_path))

# Try to import RGeoLib. If called from Grasshopper, it's already loaded.
# If running standalone, try to load from C:\geolib or dlls/main
try:
    import RGeoLib as RGL
except ImportError:
    # Fallback: try to load from C:\geolib (used by Grasshopper/Rhino)
    try:
        clr.AddReference(r"C:\geolib\_requiredDLLs\RGeoLib")
        import RGeoLib as RGL
    except:
        # Last resort: try from dlls/main if it still exists
        try:
            dll_path = libs_path / "RGeoLib"
            clr.AddReference(str(dll_path))
            import RGeoLib as RGL
        except Exception as e:
            logger.error(f"Failed to load RGeoLib: {e}")
            raise
